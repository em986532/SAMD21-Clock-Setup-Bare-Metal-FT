...
void __attribute__((weak, used, optimize("-O1"), long_call, externally_visible)) Default_Handler(void);
...
Reset_Handler(void)
{
    ...
    __extension__ ({ __asm__ volatile ("ldr r7, =_stack"); });
    __extension__ ({ __asm__ volatile ("mov sp, r7"); });

#ifdef SCB_VTOR_TBLOFF_Msk
    /* Buy stack for locals */
    __extension__ ({ __asm__ volatile ("sub sp, sp, #8"); });
#endif
    __extension__ ({ __asm__ volatile ("add r7, sp, #0"); });
#endif
...
*/
void __attribute__((weak, optimize("-O1"), section(".text.Default_Handler"),
